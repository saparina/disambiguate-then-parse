...
def process_interpretation_file(
    ...
    statistics = {
        "total_examples": len(processed_results),  # Start from number of loaded results
    ...
    for example_idx, example in enumerate(tqdm(examples)):
        ...
        for interp_idx, interp in enumerate(result["interpretations"]):
            try:
        ...
        try:
            ...
            # Track statistics for averaging
            ...
            # Get ambiguity type from the example (same as above)
            main_key = "ambig" if example.get("is_ambiguous", True) else "unambig"
            ambig_type = example.get("ambig_type", "total")
            ...
            # Track zero values for failed cases
            all_num_unique_results.append(0)
            all_matched_gold_sql.append(0)

        # Add example-level statistics
    ...
    # Calculate averages
    statistics["avg_interpretations"] = np.mean([len(interp["interpretations"]) for interp in examples])
...
def construct_output_filename_sql(args, interpretations_model_name: str) -> str:
    """Construct output filename that includes both model names"""
...
def main():
    ...
    # Extract parameters from interpretations filename
    filename_params = extract_params_from_filename(args.interpretation_file)
    ...
    # Prepare output path
    output_file = construct_output_filename_sql(args, interpretations_model_name)
